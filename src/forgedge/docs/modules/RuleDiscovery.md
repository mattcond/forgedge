...
| `purge_bars` | `None` (auto) | **campo prima non documentato.** `None` dimensiona automaticamente il purge dal `target_h` più grande del grid risolto — previene che una entry aperta a fine finestra di train venga valutata su prezzi della finestra di test adiacente; `0` disabilita il purging. Il termine `buy_delay_bar` si somma solo quando il `BacktestParams` base ha `buy_type="limit"` (fix #278) — sotto `buy_type="market"` il fill avviene sempre a `signal + 1` barra e `buy_delay_bar` non viene mai letto da `_scan_fill`, quindi sommarlo comunque avrebbe eroso la finestra utile senza motivo legato alla meccanica d'ordine effettiva (misurato: un `buy_delay_bar` alzato da 6 a 191 sotto `"market"` faceva scendere una finestra walk-forward da 2193 a 6 trade sopravvissuti, a `target_h` invariato). `GridSpec` non fa mai variare `buy_type` come asse della griglia, quindi il controllo va fatto una sola volta sul `base_params` per l'intera chiamata walk-forward. |
