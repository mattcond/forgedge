...
- **Uso di memoria scalabile con la larghezza della KPI table, non protetto dallo stesso ottimizzatore di `EventDiscovery.run()`.** `_precompute_transforms()` (§3.4) calcola `TransformLayer().transform_all()` in un unico passaggio e tiene **ogni** serie trasformata in memoria per l'intera durata della ricerca (`_series_by_col`) — necessario perché i controlli di ridondanza (§2, principio 4) e il recupero dei figli rolling (§3.6) devono poterle rileggere a ogni profondità di ogni catena. A differenza di `EventDiscovery.run()` stesso (che dal fix #232 genera e scarta i candidati grezzi in modo interlacciato, per non tenere mai l'intera popolazione pre-gate in memoria), questo modulo non può adottare la stessa strategia: le serie non sono "generate e poi scartate", sono un cache che serve fino alla fine. **Osservato concretamente**: aggiungendo 8 nuovi indicatori opt-in (CCI/WILLR/Stochastic/WMA/TRIMA/ADX/Aroon/A-D) a una KPI table oraria già ricca (123→139 colonne), due sessioni `StepWiseDiscovery` lanciate in parallelo su un container da 16 GB hanno esaurito la memoria disponibile — una è stata terminata dall'OOM killer del kernel, l'altra (partita per prima, con più memoria già allocata) ha completato normalmente. Eseguita da sola, la stessa identica sessione ha completato senza problemi. Non è un bug né un leak: è il costo di design di tenere l'intera cache di trasformazioni viva per riuso — pianifica sessioni sequenziali, non concorrenti, su KPI table larghe (indicatori opt-in multipli abilitati) a timeframe fine, o assicurati margine di memoria proporzionato al numero di colonne della KPI table prima di parallelizzare.
