...
| `experiment_en.md` | Experiment: usage guide for `StepWiseDiscovery` — a partition & compose search that grows a hold-out-confirmed rule one AND-condition at a time on top of `forge()` |
...
`forgedge.experiment` — a third sibling that, unlike the two above, RUNS its own multi-stage pipeline on top of `forge()` rather than only reading or acting on one — is also covered in the main manual (`docs/manual-en.md`, §9). [`experiment_en.md`](experiment_en.md) is its usage reference; [`modules/Experiment.md`](../modules/Experiment.md) covers the design rationale, including the manual research it generalizes.
