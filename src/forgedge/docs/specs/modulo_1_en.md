...
> additional features are present in the code (SQL export, human-readable
> math formula, OOS apply, detailed walk-forward) that are not documented.
> Updated to `main` after commits `f21b454` (5 bug fixes) and `88908ce` (`event_formula`).
...
| `event_formula` | `str` (property) | Human-readable mathematical formula (standard notation) |
...
| `event_formula` | `str` | Human-readable mathematical formula (standard notation) |
...
### 7.1 Human-readable mathematical formula (`event_formula`)

Each `EventComponent` carries an `event_formula` field:
a representation using standard mathematical notation, generated by the builder
functions `_formula_feature`, `_formula_transform`, `_formula_condition`
in `event_generator.py`.

The build logic mirrors the SQL builder's structure:

| Phase | Function | Example outputs |
|---|---|---|
...
The `EventCandidate.event_formula` property joins component formulas with ` AND `.
Included in `to_dict()` serialisation.
...
## 8. Robustness notes (bug fixes `f21b454`)

Correctness fixes that do not change the public interface but alter behaviour
in edge cases. Relevant for understanding runtime behaviour.

| Bug | File | Behaviour before | Behaviour after |
|---|---|---|---|
| **#1 — ±inf in ratio/spread** | `feature_generator.py`, `models.py` | Division by zero produced `±inf`, upcasting `float64→object` via `pd.NA` | `±inf` replaced with `float('nan')` — dtype stays `float64` |
| **#2 — float32 in ANDComposer** | `and_composer.py` | `float32` accumulators caused precision error in `max_conc` calculation | Accumulators promoted to `float64` on both pair and triple paths |
| **#3 — `diffnorm_std=0`** | `models.py` | `if not std` conflated `None` with `0.0`; `std=0` raised `KeyError` with misleading message | `if std is None` for missing-key check; `std==0` returns all-NaN series instead of error |
| **#4 — Index alignment in ConsistencyGate** | `consistency_gate.py` | Groupby with `DatetimeIndex` vs `RangeIndex` caused silent misalignment | Groupby on `.values` for both (`active` and `periods`) — positional alignment |
| **#5 — NaT crash in `pd.period_range`** | `discovery.py`, `consistency_gate.py` | All-NaT input crashed at `pd.period_range` | Null-guard on `p_min`/`p_max` before calling `period_range`; returns 0/empty Series |
...
## 11. Alignment with the functional analysis
...
- **`event_formula`** on `EventComponent` and `EventCandidate` — human-readable math formula in standard notation (parallel to `sql_expression`)
