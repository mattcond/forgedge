...
| `purge_bars` | `None` *(auto)* | Larghezza di purge, in barre, alla fine di ogni train window — le entrate aperte lì potrebbero riempirsi/uscire dentro la finestra di test adiacente, facendo trapelare la selezione di train nei prezzi di test. `None` (default) dimensiona automaticamente in base al `target_h` più grande della grid risolta, più `buy_delay_bar` **solo quando il `BacktestParams.buy_type` di base è `"limit"`** (fix #278 — sotto `"market"` il fill avviene sempre a `signal + 1` barra e `buy_delay_bar` non viene mai letto, quindi sommarlo comunque avrebbe eroso la finestra utile senza un motivo legato alla meccanica d'ordine reale); `0` disabilita il purge |
