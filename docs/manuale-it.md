...
Versione coperta: `forgedge==0.3.0`. Ogni esempio di codice in questo manuale è stato eseguito realmente contro la libreria presente in questo repository; ogni numero citato da un'esecuzione è un valore reale e verificato, non un'illustrazione inventata. Quando il manuale afferma qualcosa che gli autori stessi hanno documentato (a differenza di qualcosa dedotto dalla lettura del codice), lo dichiara esplicitamente.
...
- **Ricerca su microstruttura sub-secondo/tick-level.** Riguarda il *tipo di dato* — singoli eventi di trade/quote (microstruttura dell'order book, dinamica dello spread bid-ask) — non quanto sia breve l'intervallo della barra. Le barre OHLCV sub-orarie (15m, 5m, ...) non rientrano in questo caso: `forge_preset()` calibra una classe dedicata di griglia di orizzonti apposta per loro (internamente chiamata `"hft"` in `_TFClass`, accanto a `"daily"` e `"intraday"`, §7/§9), e ogni campo scalato sul timeframe nel resolver le tratta come caso di prima classe, non come qualcosa semplicemente tollerato. Ciò che non è testato è il dato sotto il livello di barra aggregata: ogni esempio funzionante, ogni calibrazione statistica, e lo studio di robustezza a bassa frequenza della libreria stessa (`docs/analysis/lowfreq_robustness.md`) sono costruiti su dati a barre da 1 ora fino a 1 giorno, e nessuno di essi dice nulla sui flussi tick. Nulla nel codice vieta tecnicamente l'uso a livello tick, ma vedi §16 (Trade-off) e §21 (Troubleshooting) per i problemi di potere statistico che compaiono quando la finestra in-sample non copre abbastanza *mesi di calendario* — un rischio che dipende da quanta storia hai, non da quanto è veloce l'intervallo della barra; una tabella a 15m che copre due anni ha la stessa profondità mensile di una oraria che copre due anni.
...
```python
import forgedge
print(forgedge.__version__)   # "0.3.0" al momento della scrittura
```
...
4. Ogni candidato è passato attraverso il **Consistency Gate**: si attiva abbastanza spesso (`min_tpm`), in modo abbastanza consistente tra i mesi (non bursty — `dispersion_margin` nella modalità di conteggio `"episode"` di default, `max_dispersion` in modalità `"bar"`; i due campi non si sovrappongono, #205), e con abbastanza osservazioni totali da essere statisticamente significativo? I candidati che falliscono vengono scartati silenziosamente — non diventano mai oggetti `EventCandidate`.
...
- `gate_params=GateParams()` — le soglie di default del Consistency Gate: `min_tpm=0.5` (almeno 0.5 "episodi" qualificanti al mese), `event_counting="episode"` (§15), `dispersion_margin=1.3` (le attivazioni non devono raggrupparsi più di 1.3x quanto produrrebbe un processo di Poisson al tasso osservato — il campo che governa davvero la dispersione in questa modalità di default; `max_dispersion=1.5` è presente ma non letto qui, conta solo sotto `event_counting="bar"`, #205).
- `max_and_components=1` (issue #254 Fase 8 — era `2`) — Event Discovery qui non ha composto alcuna coppia; la composizione AND propria di `forgedge` ora avviene a valle, di default, nella composizione guidata dal grado di Alpha Discovery (§8, esempio Modulo 2, step 8), non dentro Event Discovery. Alza questo valore a `2`/`3` solo per il percorso legacy single-pass (`forge(two_pass_composition=False)`) o usando `EventDiscovery` in modo standalone, come in questo esempio, volendone la propria composizione AND strutturale (§8, esempio Modulo 1, step 5).
...
8. Eseguito Rule Discovery su tutti i 392 contratti promossi con `selection_mode="walk_forward"` (il default) — cioè i parametri operativi pubblicati provengono solo dall'interno delle finestre di train walk-forward, mai da uno sguardo alla finestra di test finale.
...
5. **Consistency Gate** (§10, §15) — ogni candidato evento grezzo deve superare un filtro di rate/dispersione prima di diventare un `EventCandidate`. Gli eventi singoli che superano il gate vengono poi combinati a coppie (o triple, se `max_and_components=3`) con AND, e ogni composizione viene ripresentata allo stesso gate — solo le composizioni che a loro volta lo superano diventano candidati AND-composti. **Questo sotto-step di composizione gira solo quando `max_and_components>1`, il che dall'issue #254 Fase 8 non è più il default** (`DiscoveryConfig()` ora ha default `max_and_components=1`, composizione disattivata): il percorso di default di `forge()` (`two_pass_composition=True`) compone a valle invece, dopo la prima passata di grading di Alpha Discovery — vedi il nuovo step 8 nell'esempio del Modulo 2 più sotto. Quanto segue descrive il percorso legacy (ancora pienamente supportato), attivo quando si alza `max_and_components` manualmente — su `EventDiscovery` usato standalone, come negli esempi §6-§7 di questo manuale, o via `forge(two_pass_composition=False)`. "Lo stesso gate" è letterale, non solo le stesse soglie: la ri-sottomissione batch di `ANDComposer` e il percorso a singolo evento chiamano entrambi un'unica decisione di gate condivisa, sensibile alla modalità — così una coppia/tripla composta viene giudicata su rate/conteggio/dispersione per episodio sotto il default `event_counting="episode"` esattamente come lo sarebbe un evento singolo, e non da una reimplementazione separata solo a barra (#226). Quel calcolo batch per episodio è a sua volta vincolato in memoria indipendentemente dalla lunghezza del dataset (#228): con un gate M1 permissivo (un `min_tpm` basso, incluso il default della libreria stessa) la maggior parte di un batch di candidati supera il pre-filtro economico e raggiunge il costoso calcolo per episodio a piena dimensione, quindi `ANDComposer` dimensiona i propri batch interni a partire da un budget di memoria fisso invece che da una costante calibrata su storie brevi — un dettaglio implementativo, non qualcosa da configurare, ma il motivo per cui un run pluriennale su dati orari-o-più-fini non rischia un crash per esaurimento di memoria che uno più breve non rischierebbe. Sotto lo stesso gate M1 permissivo, anche *quali* coppie/triple specifiche riempiono `_MAX_PAIRS`/`_MAX_TRIPLES` era strutturalmente distorto (#230): l'ordine di enumerazione pre-fix esauriva tutte le coppie che coinvolgevano un singolo evento del pool prima di provarne mai un secondo, quindi tutte le 2000 coppie trattenute potevano condividere lo stesso componente — permutare quell'ordine (con un seed fisso, così gli eventi composti restano riproducibili run dopo run) ha risolto il problema, e `ANDComposer.compose()` ha anche guadagnato un parametro opzionale `max_constituent_jaccard` per rifiutare una coppia i cui due costituenti si sovrappongono troppo, a costo zero. Generare i candidati grezzi (step 4 sopra) e sottoporli al gate erano due passaggi completamente separati — la serie di attivazione di ogni candidato grezzo restava in memoria per tutto il run, anche per quelli che fallivano il gate — il che su una KPI Table ricca poteva esaurire la memoria disponibile prima ancora che M1 finisse (#232): misurato 9,5 GB per le sole serie di 50.874 candidati grezzi su una tabella a 186 colonne, prima che il gate ne liberasse anche solo uno. Generazione e gate sono ora interlacciati per batch, liberando subito la serie di un candidato che fallisce (4,2x meno memoria trattenuta, misurato sulla stessa tabella) — `DiscoveryConfig.retain_raw_events` **ha default `False` dalla Fase 8** (era `True`) e libera la serie di attivazione di un candidato che fallisce il gate non appena viene gateizzato, invece di mantenere in memoria l'intera popolazione pre-gate per tutta la durata del run (~2x RSS di picco più basso end-to-end su scala realistica, oltre alla cifra 4,2x sopra); impostarlo esplicitamente a `True` su una `DiscoveryConfig` passata a `TargetOptimizer`, che legge `EventDiscovery.raw_events` direttamente e fissa questo valore da sé sulla propria config di fallback — il percorso standard di `forge()` non legge mai quella proprietà.
...
**`GateParams.tpm_mode="ranged"`** (`docs/analysis/ranged_tpm_and_market_alignment_proposal.md` §2, opt-in, solo conteggio `"episode"`). Ogni modalità del gate vista finora tratta `min_tpm` come un floor a senso unico — "almeno così spesso." Impostare `tpm_mode="ranged"` lo trasforma nel **centro di una banda** invece: un evento che si attiva *troppo spesso* viene ora rigettato anch'esso, non solo uno troppo raro. La banda è `[min_tpm - tolleranza, min_tpm + tolleranza]` (troncata a 0); `tpm_tolerance` lasciato a `UNSET` (il default) deriva la mezza-larghezza dalla tolleranza di dispersione della sessione stessa invece che da un valore che scegli tu — `σ = sqrt(eff_max_dispersion · min_tpm / n_total_months)`, `tolleranza = 1.959964·σ` (il quantile normale a due code al 95%), riusando lo stesso `eff_max_dispersion` di sessione già calcolato dal criterio di burstiness (deliberatamente quello *di sessione*, non l'`episode_id` del singolo evento — accoppiarlo a quest'ultimo premierebbe un evento già bursty con più tolleranza sul tasso, il verso sbagliato per un filtro di qualità). Fissa `tpm_tolerance` esplicitamente invece per una banda letterale senza alcuna statistica coinvolta — `min_tpm=4.0, tpm_tolerance=2.0` significa sempre `[2.0, 6.0]`, su qualunque dataset. Un evento con `n_episodes == 0` viene sempre rigettato indipendentemente dalla banda; il criterio di burstiness esistente (`episode_id <= eff_max_dispersion`) non è toccato — la modalità ranged aggiunge un vincolo sul *livello* del tasso, non rilassa né sostituisce quello sulla sua *regolarità*. Combinare `tpm_mode="ranged"` con `event_counting="bar"` solleva `ValueError` — la modalità è definita solo per il conteggio per episodio.
...
n_total_months = 29.37   # la lunghezza di sessione del fixture ADA di questo manuale, §7/§9
p = GateParams(tpm_mode="ranged")   # min_tpm=0.5, dispersion_margin=1.3 (default)
eff = _eff_max_dispersion(n_total_months, p.dispersion_margin)
lo, hi = _tpm_band(p, eff, n_total_months)
```

**Verificato**, sulla stessa lunghezza di sessione del fixture ADA di questo manuale: `eff_max_dispersion == 1.9146`, banda derivata al `min_tpm=0.5` di default è **`[0.146, 0.854]`** episodi/mese — circa ±0.35 attorno al floor, su una sessione così breve. Imposta invece `GateParams(tpm_mode="ranged", min_tpm=4.0, tpm_tolerance=2.0)` e la banda è esattamente `[2.0, 6.0]`, indipendentemente dalla lunghezza della sessione, come promette la formula a banda letterale.
...
8. **Composizione guidata dal grado** (issue #254, `forge(two_pass_composition=True)`, il default dalla Fase 8) — gli step 1-7 sopra girano effettivamente *due volte* sotto la configurazione di default di `forge()`. La prima passata assegna un voto A-D a ogni candidato evento 1D del Modulo 1 (la lettera dello step 6, dal composite score). `forgedge.composition.grade_guided_compose()` accoppia (e, con `GradePairingConfig.max_components=3`, mette in tripla) i candidati votati usando quella lettera come criterio di pairing — stesso grado per primo, poi un grado adiacente via uno schema radice+partner (default A↔{A,B}, B↔{B,C}, C↔{C,D}) — invece del criterio puramente strutturale di tpm/dispersione/`transform_key` del Modulo 1. Il caso empirico a supporto (AMZN 1D, `docs/analysis/issue_254_two_pass_composition_plan.md`): il pairing strutturale produceva 256 candidati e 5 PARTIAL-EDGE/EDGE (1,95%); il pairing guidato dal grado sugli stessi dati ne produceva ~1400+ e 122 (~8-9%) — il voto, che incorpora già informazione sul ritorno futuro, si è rivelato un segnale di pairing molto più informativo della correlazione strutturale. I candidati composti ottengono `event_id` nuovi e vengono valutati da zero da una seconda passata indipendente di Alpha Discovery (di nuovo gli step 1-7) — nulla viene ereditato dal voto o dal target derivato di un costituente. L'output pooled di quella seconda passata (candidati composti più, di default, i singoli originali — `GradePairingConfig.include_singles_in_pass2=True`) è ciò su cui opera realmente ogni modulo a valle (il ledger delle ipotesi, il null di rotazione, il Modulo 3, il Modulo 4); `ForgeResult.grading_candidates`/`.grading_contracts` mantengono disponibili per audit gli artefatti pre-composizione della prima passata, e `ForgeResult.composition_timing` riporta il costo in wall-clock di ogni stadio. Passa `two_pass_composition=False` per saltare del tutto la prima passata e tornare alla composizione single-pass propria del Modulo 1 (step 5 sopra) — la validazione delle Fasi 5/6 ha trovato che il miglioramento del tasso di edge del design a due passate generalizza su ogni asset 1D testato (8/8) senza essere peggiore su 1H, il che è il motivo per cui è il default invece di un'opzione.
9. **Etichettatura di allineamento al mercato e la via di promozione AUC** (`docs/analysis/ranged_tpm_and_market_alignment_proposal.md` §3-§4, attiva di default da quando è stata implementata). Il test di significatività BH-FDR dello step 1 (`h_sig`) guarda un orizzonte alla volta — un edge reale ma diffuso su tutta la griglia, senza un singolo orizzonte individualmente significativo, viene perso allo stesso modo in cui un candidato con un picco isolato ma rumoroso a un solo orizzonte passerebbe. Due aggiunte, calcolate per **ogni** candidato indipendentemente dal fatto che una direzione sia mai stata assegnata, e che non toccano mai `lift` né il voto basato su `lift` — un diagnostico di allineamento al mercato sovrapposto a ciò che lo step 1 già fa, non un suo sostituto:
   - **Stadio (a), test di significatività sull'intera griglia.** Integra il tasso di eccesso *per barra*, pesato a trapezio (`Δ_h / h` — il solo `Δ_h` cumulato sembrerebbe spuriamente "momentum" per qualunque edge persistente ma piatto, per pura aritmetica del rendimento cumulato) sull'intera griglia degli orizzonti e lo testa contro la stessa infrastruttura di rotation-null che `h_sig` già usa (`p_auc`, `PromotionThresholds.auc_max_p`, default `0.10`). Un candidato può ora essere promosso da **uno dei due** test — `AlphaContract.promotion_route` registra quale: `"z_score"` (solo BH-FDR), `"auc"` (solo lo stadio (a) — il caso che il solo test dello step 1 avrebbe lasciato `"undetermined"`), `"both"`, o `None` (nessuno dei due, oppure `direction=="undetermined"`). Per la via `"auc"` da sola, `h*` **non** è `argmax|z_h|` (quel profilo è per costruzione piatto e diffuso, quindi l'argmax può cadere su un punto poco rappresentativo) ma una regola "elbow": il primo orizzonte in cui il tasso marginale per barra smette di muoversi nella stessa direzione del trend dell'intera griglia, camminando verso l'esterno dall'orizzonte più corto.
   - **Stadio (b), direzione del profilo — calcolato solo quando lo stadio (a) è significativo.** La pendenza di Spearman (`rho`, soglia `PromotionThresholds.rho_momentum_threshold`, default `0.5`) del tasso per barra lungo la griglia degli orizzonti etichetta `AlphaContract.nature`: `"momentum-aligned"` (il tasso cresce con l'orizzonte), `"mean-reversion-aligned"` (il tasso si affievolisce/inverte), `"idiosyncratic"` (nessuna pendenza chiara), o `"non_significativo"` (lo stadio (a) non era significativo — il default, e sempre il caso sulla via `"z_score"` da sola).
   - **`horizon_at_boundary`** (`bool`, replicato su `AlphaContract` accanto a `nature`/`promotion_route`) risponde a "`h*` è ancorato al bordo della griglia testata per una ragione che regge al controllo, o è genuinamente a un picco interno?" — su `"z_score"`/`"both"` richiede due incrementi consecutivi del punteggio avvicinandosi all'ultimo orizzonte con punteggio finito della griglia (un solo incremento è trattato come rumore); su `"auc"` è il flag di bordo della stessa regola elbow. Controintuitivamente, **non** declassa `nature` — empiricamente correla *di più* con i candidati genuinamente `"momentum-aligned"`, non di meno (un vero trend-follower è, per definizione, un profilo che non ha smesso di crescere quando la griglia finisce) — trattali come due fatti indipendenti e combinabili su un contratto, mai come una gerarchia.

   **Verificato**, `forge_preset("balanced", timeframe="1D")` sul fixture ADA, pipeline `forge()` di default completa (2162 contratti totali):
   ...
   # {'mean-reversion-aligned': 284, 'non_significativo': 145,   # == i 145 contratti solo "z_score"
   #  'idiosyncratic': 75, 'momentum-aligned': 15}               # 284+75+15 == 313+61, la popolazione auc/both

   # h* e tasso di bordo, per via di promozione:
   ...
   I 61 contratti solo "auc" sono esattamente quelli che `require_significant_direction=False` avrebbe lasciato `"undetermined"` prima — la chiamata `forge()` di default su questo asset ora deriva una direzione, un `nature`, e (per 284+75+15=374 di essi) un'etichetta di allineamento al mercato che prima non derivava, senza alcun cambiamento nella derivazione di `direction` per ogni altro contratto.
...
### Alternativa al Modulo 2 — `TargetOptimizer`

Un secondo modo, standalone, di arrivare al Modulo 2, per la domanda inversa: non «cosa predice questo evento?» ma «quali eventi predicono *questo* target, fissato in anticipo?». Fornisci tu il target — un orizzonte e un rendimento minimo — e `TargetOptimizer` mina direttamente gli atomi grezzi e le composizioni AND di Event Discovery contro quel target, usando uno score di lift a due proporzioni invece della pipeline derive-per-evento di Alpha Discovery.

```python
from forgedge import TargetOptimizer
from forgedge.alpha_discovery.models import TargetConfig

cfg = TargetConfig(horizon=10, min_return=0.05, side="long")   # +5% entro 10 barre, long
opt = TargetOptimizer(train_df, cfg)
# la DiscoveryConfig propria di opt, quando non passata esplicitamente, ha
# default DiscoveryConfig(train_ratio=1.0, max_and_components=2, retain_raw_events=True)
# -- fissato esplicitamente (issue #254 Fase 8), disaccoppiato dai default
# di classe di DiscoveryConfig (1 / False, calibrati per il percorso
# two-pass di forge()), perché TargetOptimizer compone gli atomi prima
# che sia visto un ritorno qualsiasi e legge ed.raw_events direttamente.
ranked = opt.run() -> pd.DataFrame       # colonne: event_id, n_components, expression,
                                          # n_activations, win_rate_event, win_rate_base, lift, z_score
opt.base_rate           # win rate incondizionato del target, impostato solo dopo run()
opt.candidates           # la list[EventCandidate] sottostante, ordinata come `ranked`

oos = opt.validate_oos(full_df, top_k=10) -> pd.DataFrame   # aggiunge colonne *_oos per le righe top-k
contracts = opt.discover_alpha(config: AlphaConfig | None = None) -> list[AlphaContract]
```

Campi e default di `TargetConfig`: `horizon: int`, `min_return: float`, `side: str` (`"long"`/`"short"`, obbligatorio, nessun default), `min_activations: int = 10`, `min_lift_atoms: float = 1.0`, `min_lift_result: float = 1.0`, `target_mode: str = "proj"`, `trend_sma_mult: float = 2.0`. **Anche qui `target_mode` ha default `"proj"`** — lo stesso scoring a eccesso-sul-trend che §8/§9/§15 descrivono per `AlphaConfig` — quindi `TargetConfig(horizon=20, min_return=0.10, side="long")` senza un `target_mode` esplicito misura «batti la SMA di trend a 2×20 barre del 10%», non «il close si è mosso +10% in 20 barre». Passa `target_mode="abs"` per la lettura letterale. `min_lift_atoms`/`min_lift_result`/`min_activations` sono soglie di potatura specifiche dei due passaggi di lift-pruning interni a `TargetOptimizer` (prima e dopo la composizione AND) — la modalità a target fisso di Alpha Discovery le ignora.

**Run verificata**, sui primi 600 bar della fixture ADA (`horizon=10, min_return=0.05, side="long"`, `target_mode` di default `"proj"`):

```
run() -> 2235 righe, base_rate = 0.5775
...
validate_oos(full_df, top_k=5) — le stesse due righe, ora con colonne *_oos:
...
Due meccaniche da conoscere prima di fidarti dei numeri OOS: il passaggio di scoring di `validate_oos()` **bypassa completamente `min_activations`** (è fissato a `1` per il calcolo effettivo di lift/z; il `min_activations` configurato guida solo il flag diagnostico `low_sample_oos`, che non filtra nulla), e riassegna l'ordine delle righe di `full_df` sull'indice del target invece di allinearsi per etichetta d'indice — sicuro solo se `cand.apply(full_df)` preserva l'ordine originale delle righe di `full_df`, cosa che avviene per la pipeline standard ma va tenuta a mente se costruisci `full_df` in modo insolito.

**Il fatto che vale la pena documentare esplicitamente: `TargetOptimizer` sta fuori dal resolver di coerenza dei parametri.** `target_optimizer.py` non importa nulla da `forgedge.resolver` né da `forgedge.unset` — a differenza di ogni altro modulo toccato dall'audit #173–#185, è stato scritto prima di quel livello, o deliberatamente tenuto fuori. `TargetConfig` di per sé non ha campi `UNSET` (ogni default è un letterale concreto), quindi non ne è toccato in ogni caso — ma l'`AlphaConfig` interno che `discover_alpha()` costruisce **non passa mai** per il resolver di sessione di `forge()`, dato che `TargetOptimizer` "non tocca `forge()` né `ForgeResult`" per design. Qualsiasi campo con sentinella `UNSET` su quell'`AlphaConfig` (`horizon_grid`, `bars_per_day`, …) ricade sul default interno di `coalesce()` di `AlphaDiscovery` in ogni punto di lettura, non sul valore calibrato per timeframe che `forge()` avrebbe iniettato — su dati giornalieri o più lenti è la stessa trappola della griglia oraria che §9 e §21 descrivono per Alpha Discovery, raggiunta da una porta diversa. `discover_alpha()` inoltre **sovrascrive silenziosamente** `config.fixed_target`, `config.target_mode` e `config.trend_sma_mult` su qualsiasi `AlphaConfig` gli passi, anche se avevi impostato tu stesso `fixed_target` — è deliberato (mantiene coerenti lo scoring IS e OOS con il `TargetConfig` che ha guidato `run()`), ma significa che l'argomento `config=` non può essere usato per puntare a un target fisso *diverso* da quello già passato a `TargetOptimizer.__init__`.
...
Il quick start del §7 stampa una manciata di campi di un caso `PARTIAL-EDGE`. Qui è lo stesso caso, ma ogni oggetto scaricato per intero — il `repr()` reale di un vero `EventCandidate`, `AlphaContract` e `RuleDiscoveryResponse`, verificato sulla fixture ADA, così puoi vedere cosa contengono davvero questi oggetti invece di doverlo dedurre dalla lista degli attributi qui sopra.

```python
c, r = partial[0]   # stessa ricerca del §7
cand = next(x for x in result.candidates if x.event_id == c.event_candidate_id)
```

**`EventCandidate`** — la condizione booleana, senza ancora alcuna nozione di cosa succede dopo che si attiva:
...
Nota cosa *non* c'è: nessun rendimento, nessuna direzione, nessun win rate. `close_vol_12` (volatilità mobile a 12 barre del close) è scesa di più del cambio a 6 barre del suo 10° percentile, 88 volte in 20 mesi attivi — questo è tutto ciò che Event Discovery sa di questo pattern. `threshold_type='distributional_p10'` è la spia che `-0.018035` è stato scelto come 10° percentile di questo specifico asset, non a mano.

**`AlphaContract`** — la stessa condizione, ora con un target economico derivato e un grade statistico:
...
Leggilo come descrive il §14: `event_stats`/`cohens_d`/`t_stat` sono forti (`p≈2e-6`) — lo score all'orizzonte 12 in `derived_target.score_by_h` è il punto migliore della griglia e supera il proprio test di significatività. Ma `diagnostics` (non bloccante, pesa sul grade) segnala già un IC di Spearman debole e una ri-verifica OOS *a livello di contratto* debole — nessuna delle due blocca la promozione (`rejection_reasons` è vuoto; solo `direction=="undetermined"` l'avrebbe bloccata), ma entrambe abbassano il grade a `B` e anticipano esattamente ciò che il giudizio OOS indipendente di Rule Discovery, qui sotto, troverà.

Questo dump precede i campi `nature`/`horizon_at_boundary`/`promotion_route` dello step 9 del §8 — su un contratto catturato oggi, `h_sig=(12,)` non vuoto significa `promotion_route="z_score"` o `"both"` (a seconda che anche il test AUC sull'intera griglia dello step 9 sia passato), `nature="non_significativo"` a meno che lo stadio (a) non lo sia, e `horizon_at_boundary=False` qui dato che `h*=12` sta ben dentro la griglia, non al bordo `h=24`.

**`RuleDiscoveryResponse`** — backtest realistico, walk-forward OOS, il verdetto economico:
...
Tre cose che questo dump rende concrete e che la lista degli attributi da sola non può:

- **`n_effective=19.158` contro `total_trades=72`.** I trade di questa regola si sovrappongono molto (`mean_concurrent_positions=3.758`, fino a 12 alla volta) — i t-test e la Deflated Sharpe di `StatisticalValidation` sono calcolati contro le ~19 osservazioni effettive, corrette per concorrenza, che questo ledger fornisce davvero, non le 72 nominali. `n_effective` non tocca mai i numeri economici (`profit_factor`, `expectancy` qui sopra restano quelli veri, nominali) — solo quelli inferenziali (errore standard/gradi di libertà del t-test, `n_obs` della `deflated_sharpe`).
- **`entry_optimization.adopted=False` con `failed_condition='sharpe'`.** Il sweep a limite dello Stage 2 ha trovato `buy_drop_pct=0.008` con uno Sharpe per trade *più alto* (scatta su fill più radi e più puliti) ma un `opportunity_sharpe` **più basso** (4.365 contro il 4.498 del punto market) una volta scontato il costo di tradare meno — vedi la discussione sulla modalità d'ingresso del Modulo 3 nel §9 per capire perché è `opportunity_sharpe`, non `StatisticalValidation.sharpe_ratio`, la metrica che decide.
- **`n_splits_in_sample=3` su 4.** È `WalkForwardSplit.tests_in_sample` aggregato: tre delle quattro finestre di test walk-forward di questa regola cadono, per timestamp, prima del confine IS/OOS della sessione (lo span su cui Alpha Discovery ha derivato `derived_target`) — cioè tre dei quattro fold «OOS» sono OOS solo rispetto alla selezione dei parametri *propria* di Rule Discovery, non rispetto al target stesso, e quindi portano un'evidenza più debole di un fold che è OOS su entrambi gli assi. È un'annotazione di qualità su `WalkForwardResult`, non un gate — `consistency=1.0` e il verdetto sono calcolati esattamente allo stesso modo in entrambi i casi. Sulle finestre di test a 4 mesi di default di questa fixture, contro uno split di sessione verso fine 2024, questo 3-su-4 è tipico, non un difetto di questa regola in particolare.

**Le formule esatte dietro due numeri citati in tutto questo manuale, dato che nessuno dei due è la versione «da manuale» che il nome suggerisce:**

- **Deflated Sharpe (`StatisticalValidation.deflated_sharpe`)** è uno sconto moltiplicativo, non la DSR probabilistica di Bailey/López de Prado che alcuni lettori conosceranno: `dsr = sharpe_annualizzata × sqrt(1 − γ·ln(n_trials) / ln(n_obs))`, dove `γ ≈ 0.5772156649` è la costante di Eulero-Mascheroni e `n_obs` è il conteggio **effettivo**, corretto per concorrenza (`n_effective`, non quello nominale). È un no-op — lo Sharpe grezzo torna invariato — ogni volta che `n_trials ≤ 1` (la validazione OOS del walk-forward, §9, passa sempre `n_trials=1` perché i dati OOS non hanno preso parte alla selezione) o `n_obs ≤ e ≈ 2.718`; restituisce `nan` esplicitamente ("selection bias troppo severo per essere credibile") se il radicando sotto la radice quadrata diventa negativo.
- **`cohens_d`** (alimenta `AlphaConfig.thresholds.min_cohens_d` e il `event_stats.cohens_d` mostrato sopra) mette in pool la varianza come `sqrt((var_a + var_b) / 2)` — la media non pesata delle varianze dei due campioni, non la classica formula pooled-variance pesata per `(n-1)`. Le due coincidono solo quando i due campioni hanno la stessa dimensione; sulle dimensioni campionarie attivo/base tipicamente diseguali che Alpha Discovery confronta, questa è una differenza numerica reale, per quanto di solito piccola, rispetto al Cohen's d a varianza pooled che calcolerebbe un manuale di statistica.
...
**Il default di `reg.flat_table()` non filtra nulla.** `apply_filters=False` di default — una chiamata semplice a `registry.flat_table()` include duplicati e regole non generiche indipendentemente da `RegistryConfig.export_duplicates`/`.export_non_generic`. Quei flag valgono solo tramite `reg.flat_table(apply_filters=True)` o `reg.export(...)`/`reg.html_report(...)` (che li applicano internamente). È una scelta di design deliberata — «FORGE mostra la complessità invece di nasconderla» — ma significa che una prima chiamata a `flat_table()` con più righe del previsto non è un bug.

**Come una regola attraversa i ticker: ricalibrazione, non riuso.** Testare `RULE_ADA_01` su `BTCUSDC` non può significare applicare la soglia letterale di ADA (`-0.018035` nell'esempio sopra) ai numeri di BTC — le distribuzioni dei due asset non condividono una scala. Prima del replay, `recalibrate_candidate()` copia in profondità il candidato e, per ogni componente che porta una soglia (i componenti categoriali/binari restano invariati — non hanno una soglia distribuzionale da spostare), fa due letture contro le distribuzioni *empiriche* del ticker sorgente e di quello target:

1. Sul ticker **sorgente**: trova il percentile che la soglia originale occupa — `q = mean(serie_sorgente <= soglia)`.
2. Sul ticker **target**: legge il valore a quello stesso percentile — `serie_target.quantile(q)`.

Quindi una regola scoperta come «10° percentile del Δvol a 6 barre di ADA» viene testata su BTC come «10° percentile del Δvol a 6 barre *di BTC stesso*», non come il numero letterale di ADA. Le trasformazioni relative (`rolling_pctrank`/`rolling_zscore`) sono vicine a un no-op sotto questa ricalibrazione, essendo già auto-normalizzanti; le soglie a trasformazione `identity` si spostano per costruzione. Il candidato ricalibrato è un oggetto nuovo con l'`event_series` in cache esplicitamente svuotata, così il replay rivaluta sempre sul frame target invece di rischiare una cache stantia del ticker sorgente.

**La genericità è una classificazione a quattro livelli, guidata dal criterio `PASS`/`FAIL` a due parti che il §10 introduce già:**

| classificazione | condizione | significato |
|---|---|---|
| `GENERIC` | passa sul 100% degli altri ticker testati | funziona ovunque testato |
| `PARTIAL` | passa su ≥ `generic_ratio_threshold` (2/3 di default) ma < 100% | funziona sulla maggioranza |
| `SPECIFIC` | passa su alcuni ma < `generic_ratio_threshold` | resta per lo più legata al ticker d'origine |
| `ISOLATED` | non passa su nessuno, **oppure** nella sessione esiste un solo ticker (niente contro cui testare) | non generalizza, o non è testabile |

Ogni singolo `PASS`/`FAIL` per ticker è il criterio a due parti del §10 (`pf_other >= cross_pf_threshold AND pf_other >= retention × pf_home`); `classification`/`is_generic` sono il roll-up su ogni *altro* ticker della sessione. `CrossTickerResult.bar` registra la soglia PF esatta (`max(cross_pf_threshold, retention × pf_home)`) che quella specifica regola doveva superare su quel target specifico — riportata per ogni risultato proprio perché un lettore possa distinguere «non tradeable lì» da «ha ceduto troppo del suo edge di casa» invece di doverlo dedurre da un solo flag pass/fail.

**La deduplicazione segnala una catena, non cancella mai.** Due regole con overlap Jaccard **strettamente maggiore** di `overlap_threshold` (0.70 di default) sulle loro date di attivazione vengono confrontate per profit factor sul ticker d'origine; quella più debole (PF strettamente più basso — una parità esatta di PF non domina in nessuna direzione) viene marcata `is_duplicate=True`, `duplicate_of=<il rule_id del vicino più forte più vicino>` — il più vicino, non la singola regola più forte dell'intero registro, così che una catena ordinata per PF (`R01 ← R02 ← R03`) resti una catena invece di collassare ogni regola più debole sull'unica migliore globale. Niente viene mai rimosso da `reg.documents` o dalla `flat_table()` di default — il §14 spiega perché: una coppia segnalata come sovrapposta non è necessariamente ridondante, può essere conferma indipendente dello stesso edge via due meccanismi diversi, e la decisione di tenerla o scartarla resta tua.

### KPI Builder — costruire una KPI Table da candele grezze

Tutto il resto di questo manuale assume che tu abbia già una **KPI Table** — un `pandas.DataFrame` con una colonna `close`, una sorgente datetime, e abbastanza colonne indicatore perché Event Discovery abbia qualcosa da minare. Se quello che hai davvero sono candele OHLCV grezze da un exchange o un fornitore dati, `forgedge.kpi_builder` è la prima cosa da chiamare, prima ancora di toccare `forge()`. È un toolkit autocontenuto a tre funzioni — nulla qui parla internamente con `forge()`, e nulla in `forge()` richiede di averlo usato; puoi costruire una KPI Table con qualunque mezzo tu voglia, purché il risultato abbia `close` e un timestamp.

#### Un percorso completo, partendo da niente più che OHLCV grezzo

Questo è deliberatamente il caso più basico: candele orarie nel formato in cui un'API REST di un exchange te le restituisce — timestamp in epoch-millisecondi, nessuna colonna derivata, niente di specifico per FORGE nella forma. Le tre righe qui sotto sono mostrate solo per la forma; la tabella reale usata per l'output verificato più sotto ne ha 500.
...
})   # una tabella reale ha tante righe quante barre — 500 nella run sotto
```

Tre chiamate trasformano questo in una tabella che `forge()` può consumare:
...
print(rep.one_line())   # check opt-in sulla qualità dei dati — non blocca mai da solo, §9/§11
```

**Output verificato**, su 500 barre orarie sintetiche ma internamente coerenti costruite così:

```
build_features   : (500, 6)  -> (500, 116)   # +color, +open_dt, +109 colonne indicatore
...
Quella tabella è ora una vera KPI Table — passala direttamente a `forge()`:
...
Niente in questo è specificamente costruito per la dimostrazione: è esattamente la sequenza `build_features → candle_features → lag_features` che il Caso d'uso 2 del §12 e lo script `examples/kpi_builder_usage.py` del repository stesso seguono, eseguita qui su dati sintetici invece che reali così puoi vederla funzionare da capo a fondo senza aver bisogno di una fonte dati. Se hai dati reali con esattamente la stessa forma (una colonna timestamp più `open`/`high`/`low`/`close`, opzionalmente `volume`), questo è letteralmente tutto ciò che serve per arrivare a `forge()`.

#### Cosa fa ciascuna funzione, parametro per parametro
...
**`build_features(candles, config, *, ...)`** calcola un insieme configurabile di indicatori base (SMA, EMA, RSI, Bollinger Bands, ATR, MACD, min/max mobile, rendimenti, volatilità, max-drawdown) da OHLCV grezzo, e deriva la colonna timestamp `open_dt` che `forge()` si aspetta.

- `candles` — l'unico argomento posizionale/richiesto oltre a `timestamp_col`. Deve contenere `timestamp_col` e qualunque colonna prezzo/volume referenziata dal tuo `config`; qualunque altra cosa sul frame (colonne di bookkeeping proprie di un exchange, una colonna simbolo, …) passa invariata.
- `config` — un `dict`, un percorso a file YAML (`str`/`Path`), o `None` per il default pacchettizzato (`DEFAULT_CONFIG`, tabella sotto) — vedi *Configurare quali indicatori vengono costruiti* sotto per lo schema.
- `timestamp_col` — **richiesto, keyword-only, nessun default.** La colonna in `candles` che porta il timestamp di ogni candela. Ometterlo solleva un `TypeError` da Python stesso (è un argomento keyword richiesto); passare un nome che non è una colonna solleva `KeyError` nominando le colonne che *sono* disponibili.
- `output_timestamp_col="open_dt"` — il nome della colonna datetime che `build_features` deriva e scrive. `"open_dt"` è ciò che ogni altro modulo della libreria cerca di default; cambialo solo se hai una ragione specifica e ricordati di passare `timestamp_col="tuo_nome"` ovunque a valle ne venga richiesto uno.
- `timestamp_unit="ms"` — come interpretare `timestamp_col` **se è numerico** (epoch millisecondi di default, come la maggior parte delle API degli exchange; passa `"s"` per secondi Unix, `"ns"` per nanosecondi). Ignorato se `timestamp_col` è già un dtype datetime o una colonna stringa parseable — `build_features` rileva automaticamente in quale caso ti trovi (`pandas.api.types.is_datetime64_any_dtype` / `is_numeric_dtype`, poi ricade su un parsing generico di stringhe).
- `add_color=True` — aggiunge una colonna `color` (`+1` se `close > open`, `-1` se `close < open`, `0` su un doji), calcolata da `open`/`close` grezzi senza arrotondamento. Richiede entrambe le colonne presenti; saltata silenziosamente (con un warning) altrimenti.
- `sort_output=True` — il frame restituito è ordinato cronologicamente per `output_timestamp_col`, indipendentemente dall'ordine in cui `candles` è arrivato. `build_features` ordina anche *prima* di calcolare gli indicatori, indipendentemente da questo flag, quindi la matematica degli indicatori è sempre calcolata in ordine temporale anche se passi `sort_output=False` per saltare solo il ri-ordinamento finale.

**`candle_features(df, *, ...)`** aggiunge sei colonne di geometria candlestick scale-free — vedi la tabella sotto per cosa significa ciascuna. `order_on="open_dt"` è la colonna usata per calcolare `gap` (che richiede il close della barra *precedente*); `add_gap=False` salta quella singola colonna se non serve (es. su dati con gap di sessione reali che non vuoi trattati come geometria della candela); `round_to=5` controlla l'arrotondamento decimale su ogni colonna calcolata.

**`lag_features(df, *cols, periods=(1,2,3), like=None, order_on="open_dt")`** aggiunge copie shiftate `{col}_prev_{NN}` di colonne che **esistono già** su `df` — gira *dopo* `build_features`/`candle_features` proprio perché tu stia ritardando nomi di colonna reali e ispezionabili invece di dover predire in anticipo la convenzione di naming interna di FORGE. Due modi per selezionare cosa ritardare, usabili insieme: elenca nomi espliciti come argomenti posizionali (`lag_features(kpi, "close", "close_ema_09")`; un typo solleva `KeyError` nominando le colonne che esistono davvero), oppure passa `like="_ema_"` per ritardare *ogni* colonna il cui nome contiene quella sottostringa (`order_on` stessa non viene mai selezionata così, anche se dovesse combaciare). `periods` accetta un singolo `int` o una sequenza — `periods=[1, 2, 3]` (il default) produce tre colonne ritardate per colonna selezionata, `periods=6` ne produce una.

#### Cosa deve avere la tua tabella di input

Solo `timestamp_col` e `close` sono mai strettamente richiesti — `close` perché ogni modulo a valle misura rendimenti forward su di essa. `open`/`high`/`low` servono a pezzi specifici (`candle_features` richiede tutte e quattro le colonne OHLC e solleva `KeyError` nominando quelle mancanti; `add_color` richiede `open`+`close`; l'indicatore `atr` richiede `high`+`low` oltre a qualunque `columns` tu l'abbia configurato) ma sono altrimenti opzionali — una serie di soli `close` costruisce comunque una KPI Table valida, seppure più piccola. `volume` è opzionale ovunque: il config di default pacchettizzato la referenzia (per `moving_average`), ma una colonna che il `config` referenzia e `candles` non ha viene saltata con un `logger.warning`, mai un errore — questo è precisamente il motivo per cui un input solo-OHLC, senza alcun volume, è sempre sicuro da passare a `build_features` invariato. Se non sei sicuro che i tuoi dati grezzi siano puliti prima di spendere una run di `forge()` su di essi, `summary_report()` (usato nel percorso sopra) è il check opt-in costruito proprio per questo — vedi §9/§11 per cosa cerca e §13 per un esempio reale di come ha colto un CSV genuinamente malformato.

#### Configurare quali indicatori vengono costruiti

Un config è una mappatura semplice: `{nome_indicatore: {"enabled": bool, "params": {"periods": [...], "columns": [...]}}}`. Tre modi equivalenti per fornirne uno:

```python
kpi = build_features(candles, timestamp_col="open_time")                    # None -> default pacchettizzato
kpi = build_features(candles, {"rsi": {"enabled": True,                     # un dict semplice
                                        "params": {"periods": [14], "columns": ["close"]}}},
                      timestamp_col="open_time")
kpi = build_features(candles, "my_config.yaml", timestamp_col="open_time")  # un percorso YAML (richiede PyYAML)
```

Una chiave `"enabled"` assente ha default `True` (solo `"enabled": False` disattiva un blocco); un blocco `"params"` senza `"periods"`/`"columns"` produce liste vuote, di fatto un no-op per quell'indicatore; un nome di indicatore che la libreria non riconosce viene saltato con un warning, non un errore, quindi un typo in un config scritto a mano degrada silenziosamente invece di sollevare — controlla il log se una colonna che ti aspettavi non compare mai. Un blocco `"lagging"`, se presente (es. copiato da un config più vecchio), viene esplicitamente ignorato con una riga di log informativa — il lag ora vive in `lag_features()`, non in questo config. Il config di riferimento pacchettizzato è distribuito assieme al sorgente come file YAML modificabile e commentato (`kpi_builder/default_enricher.yaml`) con la stessa struttura che ha `DEFAULT_CONFIG` sotto — copialo come punto di partenza per il tuo file invece di scriverne uno da zero. `load_kpi_config(path)` (usata internamente quando passi uno `str`/`Path`, o chiamabile direttamente) solleva `ImportError` con un messaggio chiaro se PyYAML non è installato, e `FileNotFoundError` se `path` non esiste.
...
#### L'insieme di indicatori di default, e le sue formule esatte

Il pacchetto di default di `build_features(candles, config=None, ...)` (`DEFAULT_CONFIG`, una voce per famiglia di indicatore) è ciò che gira quando passi `config=None` — è quello che il caso d'uso 2 del §12 ha ottenuto quando ha chiesto invece una configurazione più piccola ed esplicita:

| indicatore | periodi di default | colonne di default | abilitato |
|---|---|---|---|
| `moving_average` (SMA) | 3, 9, 12, 25, 96, 168 | close, high, low, volume | sì |
| `ema` | 3, 9, 12, 25, 96, 168 | close, high, low | sì |
| `volatility` | 5, 12, 24, 96, 168 | close | sì |
| `min` / `max` | 3, 6, 12, 24, 48, 96, 168 | close, high, low | sì |
| `return` | 1, 3, 6, 12, 24, 48, 96, 168 | close | sì |
| `rsi` | 14, 25 | close | sì |
| `bollinger_bands` | 20 | close | sì |
| `max_drawdown` | 12, 24, 48, 120, 240 | close | sì |
| `atr` | 14, 28 | close (+ richiede high/low) | **no** |
| `macd` | 12, 26, 9 (fast, slow, signal) | close | **no** |

**Verificato**, su 400 barre orarie sintetiche: `build_features(candles, timestamp_col="open_time")` senza config (cioè il default pacchettizzato) non produce **nessuna** colonna `*_atr_*`/`*_macd_*` — conferma che `atr`/`macd` sono davvero spente di default. Passare una config che le abilita esplicitamente —
...
aggiunge esattamente `close_atr_14, close_atr_28, close_natr_14, close_natr_28, close_macd_12_26, close_macd_12_26_signal_09, close_macd_12_26_hist_09` — sette colonne per due blocchi di indicatori. Due dettagli da sapere prima di attivarle:

- **I due periodi di default di ATR (14, 28) sono deliberati, non arbitrari**, e il motivo è specifico a come Event Discovery accoppia le feature: a differenza di un oscillatore limitato (RSI, un `pctrank`), l'ATR grezzo non è affidabilmente scale-free da solo, quindi un *secondo* periodo esiste appositamente perché il pairing same-family `{base}_{indicatore}_{periodo}` (la convenzione sopra) possa derivare automaticamente un ratio scale-free tra i due (es. `ratio_close_atr14_atr28`) — ogni chiamata a `multiple_atr` emette anche le corrispondenti colonne `*_natr_*` (ATR normalizzato, `ATR/prezzo`) incondizionatamente, come seconda lettura già scale-free della stessa cosa. **MACD non ha un salvagente equivalente**: il suo nome di colonna porta due parametri (`fast_slow`), quindi la stessa convenzione di pairing same-family non riconosce più triple MACD come una famiglia ratio-abile nel modo in cui lo fa con più periodi ATR/EMA/SMA — passare più triple `(fast, slow, signal)` non porta lo stesso pairing automatico.
- Il parametro `periods` di `multiple_macd` è una **lista piatta di triple `(fast, slow, signal)`**, non lunghezze di finestra indipendenti come ogni altro indicatore qui — `len(periods) % 3 != 0` solleva `ValueError`. Passare `[12, 26]` aspettandosi «due finestre MACD» come faresti per RSI è il modello mentale sbagliato e farà sollevare l'eccezione.
- La **linea MACD calcolata è normalizzata per il prezzo** (`(ema_fast − ema_slow) / close`), non la differenza in unità di prezzo grezze da manuale — deliberatamente, per tenerla nella stessa famiglia scale-free del resto della KPI Table.
- ATR e RSI usano entrambi lo smoothing di Wilder (`ewm(alpha=1/period, adjust=False)`, non una media mobile semplice del true range/guadagni-perdite) — il `100 - 100/(1+rs)` di RSI usa `rs = gain/(loss + 1e-10)`, quindi si avvicina asintoticamente a 100 senza mai raggiungerlo esattamente su una finestra a perdita zero.

#### Le sei colonne di geometria candlestick che aggiunge `candle_features()`

Ciascuna è scale-free (un rapporto, non una quantità in unità di prezzo), quindi un asset da $0.01 e uno da $60.000 producono valori direttamente confrontabili:

| colonna | formula | significato |
|---|---|---|
| `body` | `(close − open) / (high − low)` | dimensione del corpo con segno — positiva su una barra rialzista, negativa su una ribassista |
| `upper_wick` | `(high − max(open, close)) / (high − low)` | ombra superiore |
| `lower_wick` | `(min(open, close) − low) / (high − low)` | ombra inferiore |
| `close_pos` | `(close − low) / (high − low)` | dove è atterrato il close nel range della propria barra, `0` al minimo, `1` al massimo |
| `range_pct` | `(high − low) / close` | ampiezza della barra relativa al prezzo |
| `gap` | `(open − prev_close) / prev_close` | gap di apertura rispetto al close della barra precedente (solo se `add_gap=True`, il default) |

Per costruzione `abs(body) + upper_wick + lower_wick == 1` su qualunque barra non piatta. Tutte e sei dividono per `(high − low)` (o per `prev_close` per `gap`), quindi una barra completamente piatta (`high == low`) o un close precedente pari a zero produce `NaN` invece di un errore di divisione — atteso su dati a bassissima liquidità, non un bug da aggirare.

#### `pattern_features()` — il livello opt-in dei pattern nominati

```python
from forgedge import pattern_features
kpi = pattern_features(kpi)   # aggiunge una colonna categoriale: candle_pattern
```

Dieci formazioni nominate a una, due e tre barre (HAMMER, DOJI, SHOOTING_STAR, MARUBOZU, BULLISH/BEARISH_ENGULFING, BULLISH/BEARISH_HARAMI, MORNING_STAR, EVENING_STAR), ciascuna una regola geometrica fissa sulle stesse frazioni body/wick che calcola `candle_features()`, coalescate in **una sola colonna categoriale** (vince il primo match, controllato in questo ordine fisso di precedenza: `morning_star, evening_star, bullish_engulfing, bearish_engulfing, bullish_harami, bearish_harami, marubozu, hammer, shooting_star, doji`) — deliberatamente *non* dieci colonne booleane separate, perché una colonna booleana sparsa per pattern avrebbe di norma ≤1 valore distinto non nullo e verrebbe scartata silenziosamente dal type classifier di FORGE prima ancora di arrivare a Event Discovery.

**Verificato**, su 400 barre OHLCV sintetiche: `candle_pattern.value_counts()` → `None: 182, DOJI: 49, SHOOTING_STAR: 41, HAMMER: 35, MARUBOZU: 33, EVENING_STAR: 31, MORNING_STAR: 20, BEARISH_HARAMI: 4, BULLISH_ENGULFING: 3, BULLISH_HARAMI: 1, BEARISH_ENGULFING: 1` — circa il 54% delle barre non porta alcun pattern nominato, e i pattern a una barra (DOJI/SHOOTING_STAR/HAMMER/MARUBOZU) dominano il resto, come atteso da regole geometriche che non richiedono conferma cross-barra. Questa funzione è **esplicitamente fuori dalla pipeline raccomandata** (`build_features → candle_features → lag_features`): il docstring del modulo inquadra la geometria continua di `candle_features()` come l'input preferito per la scoperta di soglie asset-adattive propria di FORGE, e i pattern nominati come soglie fisse, scelte a mano, più adatte a testare un'ipotesi specifica (la tabella opt-in del §15 elenca già questo trade-off). I pattern multi-barra inoltre **richiedono** `order_on` (o un `DatetimeIndex`) — a differenza di `candle_features()`/`lag_features()`, che ricadono sull'assunzione che il frame sia già in ordine cronologico, `pattern_features()` solleva `KeyError` invece di indovinare silenziosamente l'ordine delle righe per una formazione multi-barra.

#### L'altro pairing opt-in a livello di KPI: `indicator_lag_cross_lags`

Non è una funzione di `kpi_builder`, ma una leva del Modulo 1 (`DiscoveryConfig`) che ha senso solo nel contesto di cosa `kpi_builder` ha prodotto: il feature generator di Event Discovery testa sempre le colonne indicatore price-scale (famiglie SMA/EMA/WMA/HMA) contro **colonne OHLC base shiftate nel tempo** — es. `close_sma_12[t] > low[t-3]` — ai lag `DiscoveryConfig.indicator_lag_cross_lags`, default `(1, 3)`. Questo pairing è attivo di default ed è indipendente da cosa abiliti nella config di `build_features()`; passa `indicator_lag_cross_lags=()` per disattivarlo del tutto.

**Costo verificato**, su una KPI Table di 400 barre con 2 periodi EMA (una tabella piccola — il §17 riporta un costo misurato molto più grande, +24% di runtime / +21% di candidati, su una tabella a 36 colonne): `EventDiscovery(..., indicator_lag_cross_lags=(1,3))` (il default) → **6987** candidati; la stessa tabella con `indicator_lag_cross_lags=()` → **6841** candidati — una differenza reale e misurabile nella dimensione della superficie di ricerca dovuta solo a questo pairing sempre attivo, che cresce con quante colonne indicatore price-scale e periodi porta la tua KPI Table.

#### Errori comuni al primo utilizzo, da sapere in anticipo

- **Passare `timestamp_col` posizionalmente, o dimenticarlo.** È keyword-only e richiesto — `build_features(candles, timestamp_col="open_time")`, mai `build_features(candles, "open_time")`. Ometterlo del tutto solleva un semplice `TypeError` di Python (un argomento keyword richiesto mancante), non un errore specifico di `forgedge`.
- **Chiamare `candle_features()`/`pattern_features()` sulle `candles` grezze, non sull'output di `build_features()`.** Entrambe richiedono `open`/`high`/`low`/`close` — che le tue candele grezze hanno già — quindi tecnicamente gira comunque, ma perdi `open_dt` e ogni colonna indicatore, e tutto ciò che a valle si aspetta `open_dt` (incluso `forge()` stesso, a meno di passare `timestamp_col=` esplicitamente) si rompe. Incatenale in ordine: `build_features` → `candle_features` → `lag_features`.
- **Passare la KPI Table originale a `lag_features()` invece del frame su cui vuoi davvero colonne ritardate.** `lag_features()` ritarda qualunque cosa sia sul `df` che le passi — chiamala per *ultima*, dopo `candle_features()`, se vuoi anche copie ritardate delle colonne di geometria, non solo delle colonne indicatore di `build_features()`.
- **Una colonna custom o rinominata che silenziosamente non viene mai accoppiata con nulla in Event Discovery.** Non è un errore — vedi la nota sulla convenzione di naming sopra. È di gran lunga la segnalazione più comune "perché la mia feature non viene usata"; controlla il nome della colonna contro `{base}_{indicatore}_{periodo}` prima di assumere che ci sia un problema.
- **Aspettarsi colonne `atr`/`macd` dopo aver chiamato `build_features()` senza config.** Sono disabilitate nel default pacchettizzato (`DEFAULT_CONFIG` sotto) — passa un config esplicito che le abiliti, come mostrato sopra, se vuoi l'una o l'altra.
...
| `max_and_components` | `1` (era `2` pre-#254 Fase 8) | `1`=solo singoli (il Modulo 1 non compone nulla — il default, precondizione per `forge(two_pass_composition=True)`), `2`=+coppie, `3`=+coppie+triple (percorso legacy single-pass, o un chiamante con una propria fase di composizione) |
| `retain_raw_events` | `False` (era `True` pre-#254 Fase 8) | mantiene `EventDiscovery.raw_events` (popolazione pre-gate) in memoria dopo `.run()` — serve solo a `TargetOptimizer`, che lo imposta esplicitamente a `True` sulla propria config di fallback |
...
Parametri di composizione propri di `forge()` (non campi di `DiscoveryConfig` — a livello orchestratore, §9):

| Campo | Default | Significato |
|---|---|---|
| `two_pass_composition` | `True` (issue #254 Fase 8) | composizione guidata dal grado dopo la prima passata di Alpha Discovery, al posto della composizione propria del Modulo 1 guidata da `max_and_components` (§8, esempio Modulo 2, step 8). `False` riproduce esattamente il comportamento single-pass pre-#254 |
| `grade_pairing_config` | `None` → `GradePairingConfig()` | schema di pairing/budget quando `two_pass_composition=True`; vedi `GradePairingConfig` sotto |

`GradePairingConfig` (`forgedge.composition`, letto solo quando `two_pass_composition=True`):

| Campo | Default | Significato |
|---|---|---|
| `max_components` | `2` | `3` compone anche triple (#254 Fase 4) |
| `adjacency` | `{"A": ("A","B"), "B": ("B","C"), "C": ("C","D")}` | con quali gradi può accoppiarsi un grado radice — simmetrico |
| `per_stratum_pair_cap` | `100` | rappresentazione minima garantita per strato di grado (round-robin, non un tetto rigido) |
| `per_stratum_triple_cap` | `50` | idem, per le triple |
| `include_singles_in_pass2` | `True` | raggruppa i candidati 1D originali insieme a quelli composti per la seconda passata |
| `max_constituent_jaccard` | `None` | inoltrato a `ANDComposer.compose()` — rifiuta una coppia i cui costituenti si sovrappongono troppo |
...
| `tpm_mode` | `"floor"` | `"ranged"` trasforma `min_tpm` nel **centro** di una banda invece di un floor a senso unico — opt-in, solo modalità `"episode"`, §15 |
| `tpm_tolerance` | `UNSET` | mezza-larghezza della banda `"ranged"`; lasciata `UNSET`, derivata dalla tolleranza di dispersione della sessione — vedi §15 |
...
`PromotionThresholds`: `ic_min_abs=0.02`, `ic_max_p=0.05`, `min_lift=0.08`, `min_cohens_d=0.15`, `use_fdr=True`, `fdr_q=0.10`, `oos_max_p=0.10`, `min_direction_t=0.5`, `require_significant_direction=True`, `auc_max_p=0.10`, `rho_momentum_threshold=0.5` (questi ultimi due, §15, contano solo quando `require_significant_direction=True`). **Solo `direction=="undetermined"` blocca la promozione** — ogni altra soglia qui informa il voto A-D.
...
# two_pass_composition ha default True (issue #254 Fase 8) e richiede
# max_and_components <= 1 sull'event_discovery_config che riceve —
# la composizione deve avvenire in un solo posto, non in entrambi:
...
|---|---|
| `ValueError` | stringa enum-like non valida (`direction`, `target_mode`, `buy_type`, `entry_mode`, `selection_mode`, `threshold_mode`, `timeframe`, `preset`, …), campo di config numerico fuori range, argomenti mutuamente esclusivi di `forge()`, `two_pass_composition=True` con `event_discovery_config.max_and_components>1`, coppia contratto/candidato non corrispondente passata a `RuleDiscovery` |
...
# Il default GateParams(min_tpm=0.5, dispersion_margin=1.3) è già piuttosto
...
# `max_dispersion` non è la leva sulla dispersione qui — `event_counting`
# di default è "episode", dove governa `dispersion_margin` (#205).
...
| **`tpm_mode="ranged"`** (`GateParams`) | `"floor"` | `GateParams(tpm_mode="ranged")` (solo conteggio `"episode"` — solleva `ValueError` sotto `event_counting="bar"`) | `min_tpm` diventa il **centro** di una banda (`[min_tpm - tolleranza, min_tpm + tolleranza]`) invece di un floor a senso unico, così un evento che si attiva *troppo spesso* viene rigettato anch'esso, non solo uno che si attiva troppo raramente — utile per isolare pattern "a cadenza regolare" e scartare condizioni quasi sempre attive, quasi degeneri | `tpm_tolerance` lasciato a `UNSET` deriva la mezza-larghezza dalla tolleranza di dispersione della sessione (`docs/analysis/ranged_tpm_and_market_alignment_proposal.md` §2) — una sessione con storia breve o `min_tpm` basso ottiene una banda larga, che può ammettere eventi che un gate in modalità floor avrebbe rigettato come troppo rari; un evento con `n_episodes == 0` viene sempre rigettato indipendentemente dalla banda | Vuoi eventi a una frequenza target approssimativa, non "almeno" — es. calibrare una cadenza di trade attesa specifica invece di massimizzare il volume di candidati |
| **`require_significant_direction=False`** (`PromotionThresholds`) | `True` (attivo di default, opt-out) | `AlphaConfig(thresholds=PromotionThresholds(require_significant_direction=False))` | Ripristina il comportamento legacy: `argmax\|z_h\|` assegna sempre una direzione (soggetta solo a `min_direction_t`), indipendentemente dal fatto che qualche test statistico abbia trovato l'orizzonte distinguibile dalla rotation null — `statistically_weak` continua a segnalare prove deboli, ma non blocca mai | Il default (`True`) è esso stesso un vero cambio di comportamento rispetto a prima che questo venisse aggiunto: un candidato ora ha bisogno o di un orizzonte individualmente significativo per BH-FDR (`h_sig`) o di un test AUC significativo sull'intera griglia (`p_auc < auc_max_p`, §9) per ottenere una direzione — disattivarlo riapre il rischio di "coin-flip su un picco rumoroso" che quel gate esiste per chiudere | Riprodurre risultati precedenti al gate AUC, o una sessione di debug in cui vuoi vedere cosa avrebbe scelto `argmax\|z_h\|` a prescindere dalla significatività |
...
- L'esecuzione `forge()` verificata da questo manuale sul fixture ADA di 882 barre, single-thread, ha prodotto 5356 candidati → 392 promossi → 392 risposte di rule-discovery (§6). I documenti di design della libreria segnalano separatamente il Modulo 3 come lo stadio compute-intensivo su scala: "M3 sequenziale (**255 contratti × ~0.4 s** su dati piccoli)" — cioè il backtest walk-forward per contratto di Rule Discovery è il modulo il cui costo scala più direttamente con quanti contratti Alpha Discovery ha promosso.
...
### "Ho impostato `GridSpec.buy_delay_bar` (o mi aspettavo che la griglia lo tarasse) e il punto operativo non è mai cambiato"

- **Causa:** `build_grid()` fa fanning automatico di `buy_drop_pct`, `sell_pct` e `target_h` intorno al `BacktestParams` seme, formando una piccola griglia simmetrica, ma **non `buy_delay_bar`** — resta fissato al singolo valore su `base_params`, in ogni cella della griglia, a meno che tu non imposti esplicitamente `GridSpec.buy_delay_bar` a una sequenza tua. Facile da non notare perché gli altri tre assi *fanno* fanning automaticamente.
- **Conferma:** `grid_dataframe(results)["buy_delay_bar"]` ha esattamente un valore distinto anche se `buy_drop_pct`/`sell_pct`/`target_h` ne mostrano diversi.
- **Correzione:** passa `RuleDiscoveryConfig(grid=GridSpec(buy_delay_bar=(1, 3, 6)))` (o i valori rilevanti per i tuoi dati) se vuoi specificamente che venga cercato anche il ritardo di fill.

### "Uno Sharpe o una Deflated Sharpe che ho calcolato chiamando `validate()` direttamente sembra sbagliato su dati giornalieri/a 15 minuti"
...
Va divisa per tipo di dato, perché le due metà della domanda hanno risposte diverse. Le *barre* a 1 minuto restano OHLCV aggregate, calibrate esattamente come 15m/5m (`_TFClass` raggruppa tutto ciò che è sotto 1 ora in un'unica classe calibrata, internamente chiamata `"hft"`, con una propria griglia di orizzonti e una propria tolleranza di dispersione — più stretta); nulla nel codice isola 1 minuto come non supportato. Ciò che è genuinamente non testato è che nessun esempio funzionante, analisi di calibrazione, o lo studio di robustezza a bassa frequenza della libreria stessa scende sotto le barre orarie-giornaliere, quindi questo manuale non può verificare il comportamento lì da evidenza diretta. I dati tick (nessuna aggregazione in barre) sono il caso più difficile descritto nella sezione "non usare per" del §3: finestre in-sample molto brevi introducono gli stessi problemi di potere statistico che §16 e §21 descrivono per i dati giornalieri, probabilmente in modo più acuto, e nulla nel repository testa quel regime.
