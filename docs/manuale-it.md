...
2. **Generazione di feature** (`FeatureGenerator`) — deriva nuove colonne di arità 2-3 da colonne native della *stessa famiglia*: ratio (`a/b`), spread percentuali (`(a-b)/b`), differenze normalizzate (`(a-b)/σ(a-b)`), %B di Bollinger, e posizione-nel-range. Le combinazioni di arità 2 sono generate solo tra colonne della stessa famiglia semantica (due EMA, non una EMA e un RSI).
...
7. **Compilazione del contratto.** Tutti i candidati con una direzione determinata diventano oggetti `AlphaContract` con `status="HYPOTHESIS"`; ogni altra metrica sopra aggiunge solo una stringa `"[diagnostic] ..."` a `rejection_reasons` — non blocca mai la promozione. Questo è dichiarato come un principio di design deliberato: le debolezze statistiche "alimentano il voto, non scartano — Rule Discovery è l'unico giudice economico" (`src/forgedge/docs/README.md`).
...
**Convenzione di naming delle colonne, importante:** perché una colonna sia riconosciuta come parte di una *coppia ratio same-family* dal feature generator di Event Discovery, il nome deve seguire `{base}_{indicatore}_{periodo}` con `base ∈ {close, high, low, open, volume}` e `indicatore ∈ {ema, sma, rsi, dema, tema, wma, hma, mdd, atr, natr}` (o i pattern di naming dedicati per Bollinger/volatilità/rendimento). Una colonna che non rispetta questa convenzione funziona comunque come feature standalone — semplicemente non viene mai accoppiata in una feature ratio/spread. Se un indicatore custom che hai aggiunto non compare mai composto con nulla nei candidati di Event Discovery, controlla prima il nome contro questa convenzione.
...
| `walk_forward` | `None` | imposta un `WalkForwardConfig` per abilitare la validazione OOS a livello di evento (§15 — opt-in) |
...
| `walk_forward` | `WalkForwardConfig()` | `n_splits=4`, `min_train_months=6`, `purge_bars=None` (→ orizzonte testato) |
...
| **`AlphaConfig.embargo_bars` / `WalkForwardConfig.embargo_bars`** | `0` | `AlphaConfig(embargo_bars=5)` | Aggiunge un buffer di quarantena dell'autocorrelazione seriale all'inizio della finestra OOS, oltre a ciò che il purging da solo rimuove | Riduce ulteriormente il campione OOS, che — combinato con una coda OOS giornaliera già sottile — può spingere un contratto verso `INSUFFICIENT-DATA` | Asset con autocorrelazione a breve lag nota e forte oltre a ciò che la larghezza di purge (= orizzonte massimo) già copre |
