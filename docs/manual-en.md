...
- **Sub-second/tick-level microstructure research.** This is about the *data type* — individual trade/quote events (order-book microstructure, bid-ask spread dynamics) — not about how short a bar interval you use. Sub-hourly OHLCV bars (15m, 5m, ...) are not this: `forge_preset()` calibrates a dedicated horizon-grid class for them (internally named `"hft"` in `_TFClass`, alongside `"daily"` and `"intraday"`, §7/§9), and every timeframe-scaled field in the resolver treats them as a first-class case, not something merely tolerated. What actually is untested is data below the level of an aggregated bar: every worked example, every statistical calibration, and the library's own low-frequency robustness analysis (`docs/analysis/lowfreq_robustness.md`) are built around bar data from 1 hour up to 1 day, and none of it says anything about tick streams. Nothing in the code technically forbids tick-level use, but see §16 (Trade-offs) and §21 (Troubleshooting) for the statistical-power problems that appear when the in-sample window doesn't span enough *calendar months* — a risk that tracks how much history you have, not how fast the bar interval is; a 15m table covering two years has the same monthly depth as an hourly one covering two years.
...
```python
import forgedge
print(forgedge.__version__)   # "0.3.0" at the time of writing
```
...
**`GateParams.tpm_mode="ranged"`** (`docs/analysis/ranged_tpm_and_market_alignment_proposal.md` §2, opt-in, `"episode"` counting only). Every gate mode above treats `min_tpm` as a one-sided floor — "at least this often." Setting `tpm_mode="ranged"` turns it into the **centre of a band** instead: an event firing *too often* is now rejected too, not just one too rare. The band is `[min_tpm - tolerance, min_tpm + tolerance]` (clipped at 0); `tpm_tolerance` left at `UNSET` (the default) derives the half-width from the session's own dispersion tolerance rather than a value you choose — `σ = sqrt(eff_max_dispersion · min_tpm / n_total_months)`, `tolerance = 1.959964·σ` (the two-sided 95% normal quantile), reusing the same session-level `eff_max_dispersion` the burstiness criterion already computes (deliberately the *session's*, not the individual event's own `episode_id` — coupling it to the latter would reward an already-bursty event with more rate tolerance, backwards for a quality filter). Set `tpm_tolerance` explicitly instead for a literal band with no statistics involved — `min_tpm=4.0, tpm_tolerance=2.0` always means `[2.0, 6.0]`, on any dataset. An event with `n_episodes == 0` is always rejected regardless of the band; the existing burstiness criterion (`episode_id <= eff_max_dispersion`) is unaffected — ranged mode adds a constraint on the rate's *level*, it doesn't relax or replace the one on its *regularity*. Combining `tpm_mode="ranged"` with `event_counting="bar"` raises `ValueError` — the mode is only defined for episode counting.
...
n_total_months = 29.37   # this manual's ADA fixture session length, §7/§9
p = GateParams(tpm_mode="ranged")   # min_tpm=0.5, dispersion_margin=1.3 (defaults)
eff = _eff_max_dispersion(n_total_months, p.dispersion_margin)
lo, hi = _tpm_band(p, eff, n_total_months)
```

**Verified**, on this manual's own ADA fixture session length: `eff_max_dispersion == 1.9146`, derived band at the default `min_tpm=0.5` is **`[0.146, 0.854]`** episodes/month — roughly ±0.35 around the floor, on a session this short. Set `GateParams(tpm_mode="ranged", min_tpm=4.0, tpm_tolerance=2.0)` instead and the band is exactly `[2.0, 6.0]`, independent of session length, as the literal-band formula promises.
...
1. **Horizon and direction derivation.** For every horizon `h` in the (possibly enriched) grid, it computes `mean_advantage[h]` (the oriented average forward-return advantage of active bars vs. all bars) and a horizon-selection score `|mean_advantage[h]| / sqrt(h)` — a "Sharpe-like" deflation that avoids favoring short horizons just because their t-statistic denominator is smaller. `h* = argmax_h score[h]`. Direction is `"long"` if `mean_advantage[h*] > 0`, `"short"` if `< 0`, and `"undetermined"` if no horizon in the grid produces a finite, sign-determined advantage at all, if `|z_h*|` is below `min_direction_t` (default `0.5`), or — under the default `require_significant_direction=True` — if neither statistical test in step 9 below finds the edge distinguishable from noise (`False` restores the older, purely `argmax`-driven behavior).
...
9. **Market-alignment labeling and the AUC promotion route** (`docs/analysis/ranged_tpm_and_market_alignment_proposal.md` §3-§4, default-on since this was implemented). Step 1's BH-FDR significance test (`h_sig`) looks at one horizon at a time — a real edge spread thinly across the whole grid, with no single horizon individually significant, gets missed the same way a candidate with a genuinely noisy one-horizon spike gets through. Two additions, computed for **every** candidate regardless of whether a direction was ever assigned, and never touching `lift`/lift-based grading — a market-alignment diagnostic layered on top of what step 1 already does, not a replacement for it:
   - **Stage (a), whole-grid significance test.** Integrates the trapezoidal-weighted *per-bar* excess rate (`Δ_h / h` — the cumulated `Δ_h` alone would spuriously look "momentum" for any persistent-but-flat edge, purely from cumulative-return arithmetic) across the entire horizon grid and tests it against the same rotation-null machinery `h_sig` already uses (`p_auc`, `PromotionThresholds.auc_max_p`, default `0.10`). A candidate can now clear promotion via **either** test — `AlphaContract.promotion_route` records which: `"z_score"` (BH-FDR only), `"auc"` (stage (a) only — the case step 1's test alone would have left `"undetermined"`), `"both"`, or `None` (neither, or `direction=="undetermined"`). For the `"auc"`-only route, `h*` is **not** `argmax|z_h|` (that profile is flat and diffuse by construction, so the argmax can land on an unrepresentative point) but an "elbow" rule: the first horizon where the marginal per-bar rate stops moving in the same direction as the whole-grid trend, walking outward from the shortest horizon.
   - **Stage (b), direction of the profile — only computed when stage (a) is significant.** The Spearman slope (`rho`, threshold `PromotionThresholds.rho_momentum_threshold`, default `0.5`) of the per-bar rate along the horizon grid labels `AlphaContract.nature`: `"momentum-aligned"` (rate grows with horizon), `"mean-reversion-aligned"` (rate fades/reverses), `"idiosyncratic"` (no clear slope), or `"non_significativo"` (stage (a) wasn't significant — the default, and always the case on the `"z_score"`-only route).
   - **`horizon_at_boundary`** (`bool`, mirrored onto `AlphaContract` alongside `nature`/`promotion_route`) answers "is `h*` pinned to the edge of the tested grid for a reason that survives scrutiny, or genuinely at an interior peak?" — on `"z_score"`/`"both"` it requires two consecutive score increases approaching the grid's last horizon with a finite score (one increase alone is treated as noise); on `"auc"` it's the elbow rule's own boundary flag. Counterintuitively, it does **not** discount `nature` — empirically it correlates *more* with genuinely `"momentum-aligned"` candidates, not less (a true trend-follower is, by definition, a profile that hasn't stopped growing when the grid runs out) — treat the two as independent, combinable facts about a contract, never a hierarchy.

   **Verified**, `forge_preset("balanced", timeframe="1D")` on the ADA fixture, full default `forge()` pipeline (2162 total contracts):
   ...
   # {'mean-reversion-aligned': 284, 'non_significativo': 145,   # == the 145 "z_score"-only contracts
   #  'idiosyncratic': 75, 'momentum-aligned': 15}               # 284+75+15 == 313+61, the auc/both population

   # h* and boundary rate, by route:
   ...
   The 61 `"auc"`-only contracts are exactly the ones `require_significant_direction=False` would previously have left `"undetermined"` — this asset's default `forge()` call now derives a direction, `nature`, and (for 284+75+15=374 of them) a market-alignment label it did not before, with zero change to `direction`'s derivation itself for every other contract.
...
This dump predates §8 step 9's `nature`/`horizon_at_boundary`/`promotion_route` fields — on a contract captured today, `h_sig=(12,)` non-empty means `promotion_route="z_score"` or `"both"` (depending on whether the whole-grid AUC test in step 9 also cleared), `nature="non_significativo"` unless stage (a) is, and `horizon_at_boundary=False` here since `h*=12` sits well inside the grid, not at `h=24`'s edge.
...
| `tpm_mode` | `"floor"` | `"ranged"` turns `min_tpm` into the **centre** of a band instead of a one-sided floor — opt-in, `"episode"` mode only, §15 |
| `tpm_tolerance` | `UNSET` | half-width of the `"ranged"` band; left `UNSET`, derived from the session's own dispersion tolerance — see §15 |
...
`PromotionThresholds`: `ic_min_abs=0.02`, `ic_max_p=0.05`, `min_lift=0.08`, `min_cohens_d=0.15`, `use_fdr=True`, `fdr_q=0.10`, `oos_max_p=0.10`, `min_direction_t=0.5`, `require_significant_direction=True`, `auc_max_p=0.10`, `rho_momentum_threshold=0.5` (the last two, §15, only matter when `require_significant_direction=True`). **Only `direction=="undetermined"` blocks promotion** — every other threshold here informs the A–D grade.
...
| **`tpm_mode="ranged"`** (`GateParams`) | `"floor"` | `GateParams(tpm_mode="ranged")` (`"episode"` counting only — raises `ValueError` under `event_counting="bar"`) | `min_tpm` becomes the **centre** of a band (`[min_tpm - tolerance, min_tpm + tolerance]`) instead of a one-sided floor, so an event that fires *too often* is rejected too, not just one that fires too rarely — useful for isolating "regular-cadence" patterns and screening out near-degenerate, almost-always-active conditions | `tpm_tolerance` left `UNSET` derives the half-width from the session's own dispersion tolerance (`docs/analysis/ranged_tpm_and_market_alignment_proposal.md` §2) — a session with a short history or a low `min_tpm` gets a wide band, which can admit events a floor-mode gate would have rejected as too rare; an event with `n_episodes == 0` is always rejected regardless of the band | You want events at roughly a target frequency, not "at least" one — e.g. calibrating a specific expected trade cadence rather than maximizing candidate volume |
| **`require_significant_direction=False`** (`PromotionThresholds`) | `True` (default-on, opt-out) | `AlphaConfig(thresholds=PromotionThresholds(require_significant_direction=False))` | Restores the legacy behavior: `argmax\|z_h\|` always assigns a direction (subject only to `min_direction_t`), regardless of whether any statistical test found the horizon distinguishable from the rotation null — `statistically_weak` still flags thin evidence, but it never blocks | The default (`True`) is itself a real behavior change from before this was added: a candidate now needs *either* an individually BH-FDR-significant horizon (`h_sig`) *or* a significant whole-grid AUC test (`p_auc < auc_max_p`, §9) to get a direction at all — turning this off reopens the "coin-flip on a noisy peak" risk that gate exists to close | Reproducing pre-AUC-gate results, or a debugging session where you want to see what `argmax\|z_h\|` would have picked regardless of significance |
...
Split by data type, because the two halves of this question have different answers. 1-minute *bars* are still aggregated OHLCV, calibrated the same way 15m/5m are (`_TFClass` buckets anything under 1 hour into one calibrated class, internally named `"hft"`, with its own horizon grid and its own — tighter — dispersion tolerance); nothing in the code singles 1-minute out as unsupported. What's genuinely untested is that no worked example, calibration analysis, or the library's own low-frequency robustness study goes below hourly-to-daily bars, so this manual cannot verify behavior there from first-hand evidence. Tick data (no bar aggregation at all) is the harder case §3's "do not use for" section describes: very short in-sample windows introduce the same statistical-power problems §16 and §21 describe for daily data, likely more acutely, and nothing in the repository tests that regime at all.
