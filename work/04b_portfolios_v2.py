...
MAX_PER_CLASS = 2        # no portfolio may lean on one asset class
MIN_SHORTS = 1           # at least one short leg, so the book is not pure beta

def score(r):
    """v2: rank on the WEAKER of the two windows, not on the OOS window alone.

    Ranking by OOS profit factor across an 800-rule pool selects the luckiest
    OOS performer; the min() makes a rule earn its place in both windows."""
    i, o = r["m_is"], r["m_oos"]
    pf_min = min(min(o["profit_factor"], 4.0), min(i["profit_factor"], 4.0))
    pf_i, pf_o = min(i["profit_factor"], 4.0), min(o["profit_factor"], 4.0)
    stab = 1.0 - min(abs(pf_i - pf_o) / max(pf_i, pf_o), 1.0)
    ...
    n_min = min(min(o["n_trades"], i["n_trades"]) / 20.0, 1.0)
    return (0.45 * (pf_min - 1) + 0.20 * stab + 0.15 * cons
            + 0.10 * min(dsr, 2.0) / 2.0 + 0.10 * n_min)
...
def admissible(r, folio):
    if r["ticker"] in {x["ticker"] for x in folio}:
        return False
    if [CLASS[x["ticker"]] for x in folio].count(CLASS[r["ticker"]]) >= MAX_PER_CLASS:
        return False
    return not too_similar(r)

# Round 1 — draft the short leg first, while every ticker/class is still open.
# (Reserving it for the last slot fails: by then no admissible short is left.)
for pname in NAMES:
    pick = next((r for r in cands if r["alpha_id"] not in used
                 and r["direction"] == "short" and admissible(r, folios[pname])), None)
    if pick is None:
        print(f"!! no eligible short for {pname} — filling with a long instead")
        continue
    used.add(pick["alpha_id"]); folios[pname].append(pick)
    taken_by_ticker.setdefault(pick["ticker"], []).append(pick)

# Rounds 2-5 — best remaining rule, snake-drafted across the three books.
for pname in NAMES * 5:
    if len(folios[pname]) >= 5:
        continue
    pick = next((r for r in cands if r["alpha_id"] not in used
                 and admissible(r, folios[pname])), None)
    if pick is None:
        print(f"!! not enough eligible rules to fill {pname} (slot {len(folios[pname])+1})"); break
    used.add(pick["alpha_id"]); folios[pname].append(pick)
    taken_by_ticker.setdefault(pick["ticker"], []).append(pick)
...
with open(OUT / "report_v2.pkl", "wb") as fh:
